...
export function useScroll<ElType extends HTMLElement>({
  ...
  const caller = useThrottle<NonNullable<typeof onScroll>>(throttleTime);
  ...
  useEffect(() => {
    ...
    const scrollEl = meta.el instanceof HTMLHtmlElement ? window : meta.el;
  ...
  function get(): UseScrollMeta {
    const sEl = getEl();
    const x = sEl.scrollLeft;
    const y = sEl.scrollTop;
    ...
    return {
      ...
      touchLeft: x === 0,
      touchRight: (xMax - x) <= 0, // 总宽度 - 宽度 = 滚动条实际宽度
      touchTop: y === 0,
    };
  }
