...
export function getTargetDomList(target, ref) {
    if (target) {
        var targetList = isArray(target) ? target : [target];
        var ls = targetList.map(function (item) { return getRefDomOrDom(item); }).filter(function (item) { return !!item; });
        if (ls.length)
            return ls;
    }
